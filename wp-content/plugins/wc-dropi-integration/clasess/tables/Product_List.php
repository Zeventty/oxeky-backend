...
class JPIODFW_Product_List extends WP_List_Table
{
    ...
    public function process_bulk_action()
    {
        ...
        if ('import' === $this->current_action()) {
            ...
            if (!wp_verify_nonce($nonce, 'sp_import_product')) {
                ...
                $product_id = absint(sanitize_text_field($_GET['product']));
                $store_id = isset($_GET['store']) ? absint(sanitize_text_field($_GET['store'])) : null;
                $store = null;
                if ($store_id) {
                    $store_data = $this->TokenModel->getTokenById($store_id);
                    $store = $store_data;
                }

                $result = $this->ProducstInstance->import_product($product_id, null, null, null, null, null, null, null, null, null, null, null, null, null, null, $store);

                if (is_array($result) && $result['success']) {
                    $message = __('Successfully imported product', 'wc-dropi-integration');
                } else {
                    $error = true;
                    $message = is_array($result) ? $result['message'] : __('Error al importar producto', 'wc-dropi-integration');
                }
        ...
        if ((isset($_POST['action']) && $_POST['action'] == 'bulk-import')
            ...
            $store_filter = isset($_POST['store_filter']) ? sanitize_text_field($_POST['store_filter']) : (isset($_GET['store_filter']) ? sanitize_text_field($_GET['store_filter']) : null);
            $import_errors = array();

            foreach ($import_ids as $id) {
                $store = null;
                if ($store_filter) {
                    $store_data = $this->TokenModel->getTokenById(intval($store_filter));
                    $store = $store_data;
                }
                $result = $this->ProducstInstance->import_product($id, null, null, null, null, null, null, null, null, null, null, null, null, null, null, $store);
                if (is_array($result) && !$result['success']) {
                    $import_errors[] = $result['message'];
                }
            }
            ...
            if (empty($import_errors)) {
                ...
                $message = __('Errors: ', 'wc-dropi-integration') . implode(', ', $import_errors);
            }
    ...
    public function colum_img($item)
    {
        ...
        $store_name = isset($item->store->store) ? $item->store->store : '';

        $img =   isset($item->gallery[0]) ? $item->gallery[0] : [];
        if (isset($img->urlS3) && !empty($img->urlS3)) {
            $cdn_url = $store_name ? $this->ProducstInstance->getCdnUrlByStoreName($store_name) : 'https://d39ru7awumhhs2.cloudfront.net/';
            $url =  $cdn_url . $img->urlS3;

            $img = '<img style="cursor:pointer" class="img-dropi-import" width="50px" src="' . $url . '" data-src="' . $url . '"/>';
        } else if (isset($img->url)) {
            $img_url = $store_name ? $this->ProducstInstance->getImgUrlByStoreName($store_name) : $this->constants->IMG_URL;
            $url =  $img_url . $img->url;
    ...
    function extra_tablenav($which)
    {
        switch ($which) {
            case 'top':
                ...
                $link = esc_url_raw(remove_query_arg('filter_have_stock'));
...
?>
                ...
                <div class="alignleft actions">
                    <select name="category_filter" id="category-filter">
                        ...
                        <?php
                        $store_filter_cats = isset($_GET['store_filter']) ? $_GET['store_filter'] : null;
                        $all_cats = $this->ProducstInstance->getCategories($store_filter_cats);
                    ...
                    <select name="store_filter" id="store-filter">
                        ...
                        foreach ($all_stores as $key => $value) {
                            ...
                            if (isset($value->store) && $tmp != "null") {
                                printf(
                                    "<option value='%s' data-rc='%s' %s>%s</option>\n",
                                    esc_attr($value->id),
                                    esc_attr($link),
                                    selected($store_filter, $value->id, false),
                                    $value->store
                                );
                            }
                        }
